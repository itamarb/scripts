#!/bin/bash
### The root path to search has to be provided as an argument. i.e: ./dl.sh http://dl.bintray.com/avazim123/Maven/
path=$1
...
function filter {
cat wget.txt | cut -f 2 -d "\"" wget.txt | awk '{print $1}' | sed 1,4d | sed '$d' | sed '$d' | tr -d \" > paths.txt
## delete the index
rm wget.txt
}
...
cat 1/$it.txt | cut -f 2 -d "\"" 1/$it.txt | awk '{print $1}' | sed 1,4d | sed '$d' | sed '$d' | tr -d \" > 2/$it.txt
filename="2/$it.txt"
while read p; do
if [[ "$p" == *.xml ]]
then
