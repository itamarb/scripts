...
while read p; do
	path=$p
wget $path -O 5/$it.txt	
cat 5/$it.txt | cut -f 2 -d ":" 5/$it.txt | awk '{print $1}' | sed 1,10d | sed '$d' | sed '$d' | tr -d \" > 6/$it.txt
filename="6/$it.txt"
while read p; do
echo "path before file is: " $path
path2=$path$p
echo "path after after is: " $path2
echo $path2 >> 7/$it.txt
path2=$path
done < $filename
((it+=1))
...
COMMENT
<<'COMMENT'
function nextPathFromMultiFiles {
mkdir 2
mkdir 3
#mkdir 4
it=1
...
do
path=$(cat "$i") 
wget $path -O 2/$it.txt
cat 2/$it.txt | cut -f 2 -d ":" 2/$it.txt | awk '{print $1}' | sed 1,10d | sed '$d' | sed '$d' | tr -d \" > 3/$it.txt
filename="3/$it.txt"
while read p; do
if [[ "$p" == *.xml ]]
then
